...
export function FeatureSection() {
  ...
  useEffect(() => {
    const bentoNode = bentoRef.current
    const videoNode = videoSectionRef.current
    ...
    return () => {
      if (bentoNode) {
        observer.unobserve(bentoNode)
      }
      if (videoNode) {
        videoObserver.unobserve(videoNode)
      }
      if (headerNode) {
        headerObserver.unobserve(headerNode)
      }
    }
  }, [])
